...
C10B_Recvmsg c10b_recv={0};
const uint8_t c10b_datalen = sizeof(C10B_Recvmsg);
uint8_t c10b_buffer[c10b_datalen];
uint32_t c10b_active_cnt = 0;
uint8_t c10b_active_flag=0;
volatile uint8_t check1=0;
volatile uint8_t check2=0;
...
void  USART2_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) //Check if data is received //判断是否接收到数据
    {
		...
		{
			case 0:
				if( Usart_Receive==C10B_HEAD2 && Last_Recv==C10B_HEAD1 )
				{
					sm=1;
					c10b_buffer[0]=C10B_HEAD1;
					c10b_buffer[1]=C10B_HEAD2;
					recv_cnt=2;
				}
				break;
			case 1:
				c10b_buffer[recv_cnt++] = Usart_Receive;
				if( recv_cnt == c10b_datalen )
				{
					sm=0;
					if( c10b_buffer[c10b_datalen-1] == calculateBCC(c10b_buffer,c10b_datalen-1) )
					{
						c10b_active_cnt=0;
						c10b_active_flag=1;
						//解析
						memcpy(&c10b_recv,c10b_buffer,c10b_datalen);
					}
				}
				break;
		}
		Last_Recv = Usart_Receive;
    }
}
...
返回  值：无
**************************************************************************/
extern uint16_t K210_MoveX ;
extern uint16_t K210_Kp ;
extern uint16_t K210_Kd ;
extern uint16_t K210_target ;
extern uint16_t K210_aviod;
int UART4_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET) //Check if data is received //判断是否接收到数据
    {
		...
		else if( Usart_Receive=='a' )
		{
			...
			flag=!flag;
			if( flag ) Set_Control_Mode(_K210_Control);
			else Set_Control_Mode(_APP_Control);
		}			
        ...
        if(Flag_PID==2) //Analyze the data //分析数据
        {
            ...
            else  if(Receive[1]!=0x23)
            {
                ...
                switch(Receive[1])
                {
					...
					case 0x33: K210_MoveX = Data ; break; 
					case 0x34: K210_Kp = Data ;  break;         
					case 0x35: K210_Kd = Data;  break;        
					case 0x36: K210_target = Data; break;                
					case 0x37: K210_aviod = Data;break;                
            ...
            else if( Receive[1]==0x23 ) //APP上点击“发送所有数据”处理方法
            {
                ...
                if( i<=50 ) //数据在可接受范围
                {
					...
					//K210巡线调参
                    K210_MoveX = dataArray[3];
                    K210_Kp = dataArray[4];
                    K210_Kd = dataArray[5];
                    K210_target = dataArray[6];
					K210_aviod = dataArray[7];
