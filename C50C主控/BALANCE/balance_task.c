...
uint16_t K210_MoveX = 300;
uint16_t K210_Kp = 8;
uint16_t K210_Kd = 9;
uint16_t K210_target = 127;
uint16_t K210_aviod = 500;
    ...
    while(1)
    {
        ...
        if(SysVal.Time_count>CONTROL_DELAY+230)
        {
			...
			else
			{
				...
				else if (Get_Control_Mode(_K210_Control) )    K210_control();
...
extern uint32_t c10b_active_cnt;
extern uint8_t c10b_active_flag;
int flag_color=-1,tim=0;
//uint16_t K210_MoveX = 300;
//uint16_t K210_Kp = 1;
//uint16_t K210_Kd = 1;
//uint16_t K210_target = 127;
//uint16_t K210_aviod
static void K210_control(void)
{
	static float Bias,Last_Bias;
	
	//C10B活跃状态检查
	c10b_active_cnt++;
	if( c10b_active_cnt >= BALANCE_TASK_RATE )
	{
		c10b_active_flag=0;
	}
	if( c10b_recv.k210_alive && c10b_active_flag )
	{
		if( c10b_recv.color ==1 )//green
		{
			robot_control.Vx=0.6;
		}else if( c10b_recv.color ==2 )//yellow
		{
			robot_control.Vx=0.3;
		}else if( c10b_recv.color ==0 )//red
		{
			robot_control.Vx=0.1;
		}
		Bias = K210_target-c10b_recv.k210_cx;
		robot_control.Vz = (float)K210_Kp/1000.0f*Bias + (float)K210_Kd/1000*(Bias-Last_Bias);
		Last_Bias = Bias;
	}
	else
	{
...
void Xbox360GamePad_KeyEvent_Callback(uint8_t keyid,GamePadKeyEventType_t event)
{
	...
	if( keyid == Xbox360KEY_Y && event == GamePadKeyEvent_LONGCLICK )
	{
		...
		else Set_Control_Mode(_PS2_Control);
	}
...
static void UserKey_Scan(u16 rate)
{
	...
	{	
		...
		flag=!flag;
		if( flag ) Set_Control_Mode(_K210_Control);
		else Set_Control_Mode(_ROS_Control);
	}
